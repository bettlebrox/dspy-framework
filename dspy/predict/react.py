...
class ReAct(Module):
    def __init__(self, signature, max_iters=5, num_results=3, tools=None):
        ...
        instr = "\n".join(instr)
        self.react = [Predict(dspy.Signature(self._generate_signature(i), instr)) for i in range(1, max_iters + 1)]
    ...
    def _generate_signature(self, iters):
        ...
        for j in range(1, iters + 1):
            ...
            tool_list = " or ".join(
                [f"{tool.name}[{tool.input_variable}]" for tool in self.tools.values() if tool.name != "Finish"],
            )
